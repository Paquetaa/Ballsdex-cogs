...
class Bet(commands.GroupCog):
    """
    Bet countryballs with other players in Dex Bet games.
    """
    ...
    async def begin(self, interaction: discord.Interaction["ballsdexBot"], user: discord.User):
        """
        Begin a Dex Bet with the chosen user.
        ...
        await menu.start()
        await interaction.response.send_message("Dex Bet started!", ephemeral=True)
