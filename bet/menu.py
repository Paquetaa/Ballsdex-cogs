...
class ConfirmView(View):
    ...
    async def accept_button(self, interaction: discord.Interaction["ballsdexBot"], button: Button):
        ...
        if self.bet.bettor1.accepted and self.bet.bettor2.accepted:
            if result:
                await interaction.followup.send("The Dex Bet is now concluded.", ephemeral=True)
            else:
                await interaction.followup.send(
...
class BetMenu:
    ...
    def _generate_embed(self):
        ...
        self.embed.title = f"{settings.plural_collectible_name.title()} Dex Betting"
    ...
    async def start(self):
        ...
        self.message = await self.channel.send(
            ...
            "is proposing a Dex Bet with you!",
            embed=self.embed,
    ...
    async def lock(self, bettor: BettingUser):
        ...
        if self.bettor1.locked and self.bettor2.locked:
            ...
            self.embed.description = (
                ...
                "**Please confirm to start the Dex Bet!**\n\n"
    ...
    async def confirm(self, bettor: BettingUser) -> bool:
        """
        Confirm the bet. If both players confirm, execute the Dex Bet.
        """
        ...
        if self.bettor1.accepted and self.bettor2.accepted:
            ...
            try:
                ...
                log.exception(f"Error executing Dex Bet: {e}")
